...
def encrypt_file_bytes(key: bytes, orig_filename: str, plaintext: bytes) -> bytes:
    """
    Returns bytes for the encrypted package with internal metadata.
    Format:
    ...
    # encrypt metadata and file separately
...
def decrypt_file_bytes(key: bytes, package: bytes) -> dict:
    """
    Returns dict: {"metadata": meta_dict, "plaintext": bytes}
    Raises on format/auth errors.
    """
    min_len = 4 + 1 + 12 + 12 + 4
